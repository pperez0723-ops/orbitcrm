...
export default function OrbitalPipeline({ initialDeals, stages, workspaceId, pipelineId }: Props) {
  ...
  deals.forEach(d => { if (byStage[d.status] !== undefined) byStage[d.status].push(d); });
  ...
  const setSelectedCb = useCallback((d: Deal) => setSelected(d), []);
  ...
  const renderNodes = useCallback(() => {
    ...
    const sc = Math.min(W / 500, H / 320, 1.4);
    const offX = (W - 500 * sc) / 2, offY = (H - 320 * sc) / 2;
    const cx = 250, cy = 160;
    container.innerHTML = '';
    sortedStages.forEach((stage, ringIdx) => {
      ...
      list.forEach((deal, idx) => {
        ...
        const px = offX + (cx + orbit.rx * Math.cos(angle)) * sc;
        const py = offY + (cy + orbit.ry * Math.sin(angle)) * sc;
        ...
        node.addEventListener('click', () => setSelectedCb(deal));
    ...
    function loop() {
      ...
      if (svg) {
        [{id:'od1',rx:48,ry:28,speed:.5,offset:0},{id:'od2',rx:88,ry:50,speed:.32,offset:2.1},{id:'od3',rx:128,ry:72,speed:.22,offset:4.1},{id:'od4',rx:168,ry:94,speed:.16,offset:1.2},{id:'od5',rx:208,ry:116,speed:.12,offset:3.3}].forEach(d => {
          ...
          el.setAttribute('cx', (offX + (cx + d.rx * Math.cos(a)) * sc).toFixed(1));
          el.setAttribute('cy', (offY + (cy + d.ry * Math.sin(a)) * sc).toFixed(1));
        });
      }
  ...
  return (
    <div style={{position:'relative',width:'100%',height:'100%',minHeight:400,background:'radial-gradient(ellipse at 50% 60%,rgba(30,10,40,.95) 0%,#05050f 100%)',borderRadius:16,overflow:'hidden',fontFamily:"'Rajdhani',sans-serif"}}>
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,zIndex:20,display:'flex',alignItems:'center',padding:'10px 14px',background:'rgba(5,5,15,0.7)',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
        <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:'rgba(242,240,250,0.5)'}}>ORBIT PIPELINE</div>
        <div style={{marginLeft:'auto',display:'flex',gap:8,alignItems:'center'}}>
          <span style={{fontSize:11,fontFamily:'monospace',color:'rgba(242,240,250,0.35)',letterSpacing:1}}>{clock}</span>
          <span style={{fontSize:11,color:'rgba(242,240,250,0.4)'}}>|</span>
          <span style={{fontSize:11,color:'#2DD4BF',fontWeight:700}}>{totalDeals} deals</span>
          <span style={{fontSize:11,color:'rgba(242,240,250,0.4)'}}>|</span>
          <span style={{fontSize:11,color:'#F4B942',fontWeight:700}}>{fmt(totalVal)}</span>
          {wonVal > 0 && <><span style={{fontSize:11,color:'rgba(242,240,250,0.4)'}}>|</span><span style={{fontSize:11,color:'#36d399',fontWeight:700}}>Won {fmt(wonVal)}</span></>}
        </div>
      </div>

      {/* Stage filter pills */}
      <div style={{position:'absolute',top:38,left:0,right:0,zIndex:20,display:'flex',gap:5,padding:'6px 14px',overflowX:'auto'}}>
        <button onClick={() => setFilter('all')} style={{padding:'3px 10px',borderRadius:20,border:'1px solid rgba(255,255,255,0.12)',background:filter==='all'?'rgba(255,255,255,0.12)':'transparent',color:'rgba(242,240,250,0.7)',fontSize:10,cursor:'pointer',letterSpacing:.5,whiteSpace:'nowrap'}}>All</button>
        ...
        <button onClick={() => setModal(true)} style={{marginLeft:'auto',padding:'3px 12px',borderRadius:20,border:'1px solid #E8303A88',background:'rgba(232,48,58,0.15)',color:'#E8303A',fontSize:10,cursor:'pointer',letterSpacing:.5,whiteSpace:'nowrap',flexShrink:0}}>+ Add Lead</button>
      </div>
      ...
      <div ref={wrapRef} style={{position:'absolute',top:70,left:0,right:210,bottom:0}}>
          ...
          {sortedStages.map((s, i) => {
            ...
            return (
              <ellipse key={s.id}
                ...
                rx={`${orbit.rx / 500 * 100}%`}
                ry={`${orbit.ry / 320 * 100}%`}
                fill="none" stroke={col} strokeWidth="1" strokeOpacity=".2" strokeDasharray="3 6"
              />
            );
          })}
          {sortedStages.map((s, i) => {
        ...
        <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',width:44,height:44,borderRadius:'50%',background:'radial-gradient(circle,rgba(232,48,58,0.3),rgba(232,48,58,0.05))',border:'1.5px solid rgba(232,48,58,0.5)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,zIndex:5,boxShadow:'0 0 18px rgba(232,48,58,0.3)'}}>🌐</div>
      </div>

      {/* Side panel */}
      <div style={{position:'absolute',right:0,top:70,bottom:0,width:210,background:'rgba(8,8,20,0.92)',borderLeft:'1px solid rgba(255,255,255,0.05)',padding:'10px',overflowY:'auto'}}>
        <div style={{fontSize:10,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:'rgba(242,240,250,0.35)',marginBottom:8}}>Stages</div>
          ...
          return (
            <div key={stage.id} style={{marginBottom:10}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}>
                <span style={{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:1.5,color:col}}>{stage.name}</span>
                <span style={{fontSize:9,padding:'1px 5px',borderRadius:8,background:col+'22',color:col}}>{cnt}</span>
              </div>
              {sample.map((d, idx) => (
                <div key={idx} onClick={() => setSelected(d)} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 0',borderBottom:'1px solid rgba(255,255,255,0.04)',cursor:'pointer'}}>
                  ...
                  <span style={{fontSize:10,color:'#2DD4BF'}}>{fmt(d.value)}</span>
                </div>
              ))}
              {cnt > sample.length && <div style={{fontSize:9.5,color:'rgba(242,240,250,0.3)',paddingTop:3}}>+{cnt - sample.length} more</div>}
              <div style={{fontSize:10,color:'rgba(242,240,250,0.3)',textAlign:'right',paddingTop:2}}>{fmt(val)}</div>
            </div>
          );
        })}
      </div>
      ...
      {selected && (
        <div onClick={() => setSelected(null)} style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.55)',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div onClick={e => e.stopPropagation()} style={{background:'#0e0e1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:14,padding:'22px 24px',minWidth:240,maxWidth:320,boxShadow:'0 8px 40px rgba(0,0,0,.8)'}}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:6}}>{selected.name}</div>
            <div style={{fontSize:12,color:'rgba(242,240,250,0.45)',marginBottom:4}}>{selected.company || '—'}</div>
            <div style={{fontSize:20,fontWeight:700,color:'#2DD4BF',marginBottom:4}}>{fmt(selected.value)}</div>
            <div style={{fontSize:11,color:'rgba(242,240,250,0.5)',textTransform:'uppercase',letterSpacing:1}}>{selected.status}</div>
            <button onClick={() => setSelected(null)} style={{marginTop:14,padding:'6px 18px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.05)',color:'rgba(242,240,250,0.7)',cursor:'pointer',fontSize:12}}>Close</button>
          </div>
        </div>
      )}
      ...
      {modal && (
        <div onClick={() => setModal(false)} style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.55)',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div onClick={e => e.stopPropagation()} style={{background:'#0e0e1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:14,padding:'22px 24px',width:300,boxShadow:'0 8px 40px rgba(0,0,0,.8)'}}>
            <div style={{fontSize:14,fontWeight:700,marginBottom:14,letterSpacing:1}}>Launch New Lead</div>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:'rgba(242,240,250,0.4)',marginBottom:3}}>Business Name *</div>
              <input value={ldName} onChange={e => setLdName(e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.05)',color:'#f2f0fa',fontSize:12,boxSizing:'border-box'}} />
            </div>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:'rgba(242,240,250,0.4)',marginBottom:3}}>Phone</div>
              <input value={ldPhone} onChange={e => setLdPhone(e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.05)',color:'#f2f0fa',fontSize:12,boxSizing:'border-box'}} />
            </div>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:'rgba(242,240,250,0.4)',marginBottom:3}}>Category / Industry</div>
              <input value={ldCat} onChange={e => setLdCat(e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.05)',color:'#f2f0fa',fontSize:12,boxSizing:'border-box'}} />
            </div>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:'rgba(242,240,250,0.4)',marginBottom:3}}>Value ($)</div>
            ...
            <div style={{marginBottom:14}}>
              <div style={{fontSize:10,color:'rgba(242,240,250,0.4)',marginBottom:3}}>Stage</div>
              <select value={ldStage} onChange={e => setLdStage(e.target.value)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'#0e0e1e',color:'#f2f0fa',fontSize:12,boxSizing:'border-box'}}>
                {sortedStages.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
              </select>
            </div>
            {ldMsg && <div style={{fontSize:11,color:ldMsg.startsWith('Error')?'#E8303A':'#36d399',marginBottom:8}}>{ldMsg}</div>}
            <div style={{display:'flex',gap:8}}>
              <button onClick={() => setModal(false)} style={{flex:1,padding:'8px',borderRadius:8,border:'1px solid rgba(255,255,255,0.1)',background:'transparent',color:'rgba(242,240,250,0.5)',cursor:'pointer',fontSize:12}}>Cancel</button>
              <button onClick={saveLead} disabled={ldSaving} style={{flex:2,padding:'8px',borderRadius:8,border:'none',background:'linear-gradient(135deg,#E8303A,#a0001a)',color:'#fff',cursor:'pointer',fontSize:12,fontWeight:700}}>{ldSaving ? 'Launching…' : 'Launch'}</button>
            </div>
          </div>
        </div>
      )}

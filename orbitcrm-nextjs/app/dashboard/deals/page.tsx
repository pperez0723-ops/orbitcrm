...
type Deal = {id?:string;name:string;company:string;value:number;status:string};
...
export default function DealsPage() {
  ...
  useEffect(() => {
    sb.from('leads').select('id,title,company,estimated_value,status').order('created_at',{ascending:false}).limit(500)
      .then(({data}) => {
        if(data) setDeals(data.map((r:any) => ({
          id: r.id,
          name: r.title || r.name || 'Unnamed',
          company: r.company || 'lead',
          value: r.estimated_value || 500,
          status: r.status || 'New Lead',
        })));
      });
  }, []);

  const byStage = Object.keys(ORBITS).reduce((acc,s) => ({...acc,[s]:deals.filter(d=>d.status===s)}), {} as Record<string,Deal[]>);
  const totals = Object.keys(ORBITS).reduce((acc,s) => ({...acc,[s]:{count:byStage[s].length,value:byStage[s].reduce((sum,d)=>sum+d.value,0)}}), {} as Record<string,{count:number;value:number}>);
  ...
  const renderNodes = useCallback(() => {
    ...
    const cx=400, cy=250;
    container.innerHTML='';
    const visible = Object.keys(ORBITS).filter(st => filter==='all'||filter===st);
  ...
  useEffect(() => {
    ...
    (function loop(){
      ...
      const container=nodesRef.current;
      if(container){
        container.querySelectorAll<HTMLElement>('[data-rx]').forEach(n=>{
      ...
      if(svg){
        const dotCfg=[{id:'od1',rx:70,ry:40,speed:.5,offset:0},{id:'od2',rx:132,ry:74,speed:.32,offset:2.1},{id:'od3',rx:194,ry:108,speed:.22,offset:3.5},{id:'od4',rx:256,ry:142,speed:.16,offset:5.2},{id:'od5',rx:70,ry:40,speed:.5,offset:Math.PI},{id:'od6',rx:132,ry:74,speed:.32,offset:Math.PI+1.4}];
        dotCfg.forEach(d=>{const el=svg.querySelector('#'+d.id);if(!el)return;const a=t*d.speed+d.offset;el.setAttribute('cx',(cx+d.rx*Math.cos(a)).toFixed(1));el.setAttribute('cy',(cy+d.ry*Math.sin(a)).toFixed(1));});
      }
  ...
  async function saveLead() {
    ...
    try {
      const r=await fetch(FN_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({secret:FN_SECRET,name:ldName,phone:ldPhone,category:ldCat,value:ldValue,stage:ldStage})});
      const d=await r.json();
      if(d.ok){
        setDeals(prev=>[...prev,{name:ldName,company:(ldCat||'lead')+' · added',value:ldValue,status:ldStage}]);
      ...
      <div ref={wrapRef} style={{flex:1,position:'relative',overflow:'hidden',zIndex:5}}>
        ...
        <div style={{position:'absolute',left:13,top:13,width:178,background:'rgba(8,8,14,0.93)',border:'1px solid rgba(232,48,58,0.2)',borderRadius:12,padding:12,zIndex:15,backdropFilter:'blur(12px)'}}>
          <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
            <div style={{width:20,height:20,borderRadius:5,background:'rgba(232,48,58,0.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10}}>AI</div>
          ...
          <div style={{fontSize:10.5,color:'rgba(242,240,250,0.6)',lineHeight:1.65}}>
            <span style={{color:'#F2F0FA',fontWeight:600}}>{draftedCount} leads drafted</span> and waiting. Fire the SMS batch to push them to Contacted.
          </div>
        ...
        <div style={{position:'absolute',bottom:12,left:16,display:'flex',gap:5,zIndex:15,flexWrap:'wrap',maxWidth:'calc(100% - 240px)'}}>
          {([['all','All'],['New Lead','New Lead'],['SMS Drafted','SMS Drafted'],['Contacted','Contacted'],['Replied','Replied'],['Won','Won']] as [string,string][]).map(([val,label])=>(
              ...
              <div key={stage} style={{marginBottom:14}}>
                ...
                {sample.map((d,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:6,padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                    ...
                    <span style={{fontFamily:"'Rajdhani',sans-serif",fontSize:10,color:'#2DD4BF'}}>${(d.value||0).toLocaleString()}</span>
                  </div>
                ))}
                {cnt>sample.length&&<div style={{fontSize:9.5,color:'rgba(242,240,250,0.3)',paddingTop:4}}>+ {cnt-sample.length} more orbiting</div>}
      ...
      {modal&&(
        <div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(4,4,10,0.72)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)setModal(false);}}>
          <div style={{width:340,background:'#111119',border:'1px solid rgba(232,48,58,0.2)',borderRadius:16,padding:22,boxShadow:'0 30px 80px rgba(0,0,0,.6)'}}>
            ...
            <div style={{fontSize:11,color:'rgba(242,240,250,0.3)',marginBottom:16}}>Drops onto the orbit and saves to your Orbit pipeline.</div>
            ...
            <div style={{display:'flex',gap:9,marginBottom:9}}>
              <input type="number" value={ldValue} onChange={e=>setLdValue(+e.target.value)} placeholder="Value" style={{flex:1,background:'#181822',border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,padding:'10px 12px',fontSize:13,color:'#F2F0FA',outline:'none'}}/>
              <select value={ldStage} onChange={e=>setLdStage(e.target.value)} style={{flex:1.3,background:'#181822',border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,padding:'10px 12px',fontSize:13,color:'#F2F0FA',outline:'none'}}>
                {Object.keys(ORBITS).map(s=><option key={s}>{s}</option>)}
              </select>
            </div>
            <div style={{fontSize:11,minHeight:16,marginBottom:8,color:ldMsg.startsWith('Launched')?'#2DD4BF':ldMsg.includes('Error')||ldMsg.includes('error')?'#E8303A':'rgba(242,240,250,0.3)'}}>{ldMsg}</div>

...
export default function OfficePage() {
  ...
  useEffect(() => {
    ...
    function el(t, a, par) { const e = document.createElementNS(NS,t); for(const k in a) e.setAttribute(k,String(a[k])); (par||svg).appendChild(e); return e; }
    function elg(a, par) { return el('g', a||{}, par||svg); }
    ...
    for(let i=0;i<110;i++) el('circle',{cx:(65+Math.random()*770).toFixed(1),cy:(9+Math.random()*30).toFixed(1),r:(Math.random()<.12?1.3:.5).toFixed(1),fill:'#fff',opacity:(.25+Math.random()*.7).toFixed(2)});
    [[180,24,'#00e5ff',.8],[620,28,'#f0c96b',.7],[800,20,'#a78bfa',.75]].forEach(([cx,cy,cf,op]) => el('circle',{cx,cy,r:1.8,fill:cf,opacity:op}));
    ...
    ['SYS:NOMINAL','ORBIT:STABLE','CREW:6','AI:ONLINE'].forEach((s,i) => { const t=el('text',{x:55+i*218,y:67,fill:['#00e5ff','#36d399','#f0c96b','#a78bfa'][i],'font-family':"'JetBrains Mono'",'font-size':9.5,opacity:.75}); t.textContent=s; });
    ...
    const SCREENS = {};
    const GLOWS = {};
    function buildDesk(key, dx, dy, color) {
      const g = elg({}, svg);
    ...
    function setScreen(name, mode) {
      const arr=SCREENS[name]; if(!arr) return;
      const g=GLOWS[name];
      if(mode==='work'){ arr[0].setAttribute('fill','#e8303a'); arr[1].setAttribute('fill','#c02828'); if(g) g.setAttribute('opacity','0.1'); }
      else if(mode==='done'){ arr[0].setAttribute('fill','#1f9f76'); arr[1].setAttribute('fill','#147055'); if(g) g.setAttribute('opacity','0.07'); }
      else { arr[0].setAttribute('fill','#08101c'); arr[1].setAttribute('fill','#08101c'); if(g) g.setAttribute('opacity','0'); }
    }

    function makeAstro(par, color, name, role) {
      const g=elg({},par);
      ...
      const rt=el('text',{x:0,y:31,'text-anchor':'middle',fill:color+'cc','font-family':"'JetBrains Mono'",'font-size':7.5},g); rt.textContent=role;
      // Zzz
      const zg=elg({},g);
      ...
      return {g,bl,br,body,vglow,ant,zg};
    }
    // Agents
    const AG = {};
    AGENTS.forEach((a, i) => {
      ...
      AG[a.key] = {...spr, x:start.x, y:start.y, tx:start.x, ty:start.y, state:'wander', pause:Math.random()*3, phase:Math.random()*6, speed:28+Math.random()*14, meetIdx:i, key:a.key, color:a.color};
      spr.g.setAttribute('transform', `translate(${start.x},${start.y})`);
    });

    // Inject zzz CSS
    if (!document.getElementById('orb-zzz')) {
      const st = document.createElement('style'); st.id='orb-zzz';
      st.textContent = '.ozg text{opacity:0}.ozs .oz1{animation:ozf 2.2s ease-in-out infinite}.ozs .oz2{animation:ozf 2.2s ease-in-out infinite .55s}.ozs .oz3{animation:ozf 2.2s ease-in-out infinite 1.1s}@keyframes ozf{0%{opacity:0;transform:translate(0,0)}30%{opacity:.9}100%{opacity:0;transform:translate(4px,-11px)}}';
      document.head.appendChild(st);
    }
    Object.values(AG).forEach(a => { a.zg.classList.add('ozg'); a.z1 && a.z1.classList.add('oz1'); a.z2 && a.z2.classList.add('oz2'); a.z3 && a.z3.classList.add('oz3'); });

    function deskPos(key) { const a = AGENTS.find(x=>x.key===key); return {x:a.dx+27,y:a.dy+50}; }
    function pickWander(a) { const w=WPT[Math.floor(Math.random()*WPT.length)]; a.tx=Math.max(35,Math.min(865,w.x+(Math.random()*30-15))); a.ty=Math.max(90,Math.min(460,w.y+(Math.random()*20-10))); }
    ...
    function frame(t) {
      ...
      Object.values(AG).forEach(a => {
        if (a.state==='work') {
          ...
          a.vglow.setAttribute('opacity',(0.4+Math.abs(Math.sin(t/200))*0.5).toFixed(2));
    ...
    // Expose to dispatch system
    (window as any).__OS = { setS:(k,s) => { if(AG[k]) AG[k].state=s; }, pw:(k) => { if(AG[k]) pickWander(AG[k]); }, scr:setScreen };
    return () => cancelAnimationFrame(raf);
  }, []);

  // Terminal helpers
  function addRow(html) { if(!termRef.current) return null; const l=document.createElement('div'); l.style.cssText='font-family:JetBrains Mono,monospace;font-size:11.5px;line-height:1.7;padding:1px 0'; l.innerHTML=html; termRef.current.appendChild(l); termRef.current.scrollTop=termRef.current.scrollHeight; return l; }
  const C = {ok:'#36d399',wa:'#f4b942',inf:'#00e5ff',mu:'#5a8aaa',hd:'#2a4a6a',fi:'#a78bfa',pr:'#ff5d66'};
  function tOut(cls, txt) { addRow(`<span style='color:${C[cls]||'#dff0ff'}'>${txt.replace(/&/g,'&amp;').replace(/</g,'&lt;')}</span>`); }
  function tw(ms) { return new Promise(r=>setTimeout(r,ms)); }
  async function typeCmd(txt) {
    ...
    if(!l) return; const sp=l.querySelectorAll('span')[1], cur=l.querySelectorAll('span')[2];
    for(let i=0;i<txt.length;i++){if(sp)sp.textContent+=txt[i]; await tw(9+Math.random()*14);} if(cur)cur.remove();
  }
  async function tProg(lbl) {
    const l=addRow(`<span style='color:#5a8aaa'>${lbl} </span><span></span>`);
    const b=l?.querySelectorAll('span')[1];
    const bars=['░'.repeat(10),'▓▓▓'+'░'.repeat(7),'▓'.repeat(6)+'░'.repeat(4),'▓'.repeat(9)+'░','▓'.repeat(10)];
    for(let i=0;i<bars.length;i++){if(b)b.textContent=bars[i]+' '+(i*25)+'%'; if(termRef.current) termRef.current.scrollTop=termRef.current.scrollHeight; await tw(110);}
    if(b) b.innerHTML='▓'.repeat(10)+' <span style="color:#36d399">DONE</span>';
  }

  function route(o) {
    const ol=o.toLowerCase(), s={} as any;
    if(/research|find|analyz|lead|business|prospect/.test(ol)) s.astra=1;
    if(/landing|web|page|site|seo|design|ui/.test(ol)) s.orion=1;
    if(/app|automat|backend|api|sms|email|crm|trigger|contact/.test(ol)) s.luna=1;
    if(/sales|pipeline|deal|close|quota|follow.?up/.test(ol)) s.rex=1;
    if(/ops|report|analytic|metric|kpi|monitor/.test(ol)) s.vera=1;
    if(/everything|whole|full|launch|mvp|all/.test(ol)){s.astra=1;s.orion=1;s.luna=1;s.rex=1;s.vera=1;}
    const k=Object.keys(s); return k.length?k:['astra','luna'];
  }

  function agSteps(k, o) {
    const ol=o.toLowerCase();
    if(k==='astra') return [{c:'orbit leadgen --needs-website',out:[['inf','Scanning OSM + Maps…']]},{p:'Filtering'},{out:[['mu','100 biz → pipeline']]},{c:'orbit draft-sms --per-lead',out:[['ok','✓ SMS drafted']]}];
    ...
    if(k==='luna') return /sms|email|welcome/.test(ol)?[{c:'orbit fn deploy welcome',out:[['inf','Deploying…']]},{p:'Trigger'},{out:[['ok','✓ SMS+email confirmed']]}]:[{c:'orbit gen api --contacts',out:[['inf','Scaffolding…']]},{p:'Build'},{out:[['ok','✓ Preview live']]}];
    if(k==='rex') return [{c:'orbit pipeline update --bulk',out:[['inf','Syncing…']]},{p:'Updating'},{out:[['mu','8 deals → Proposal']]},{out:[['ok','✓ 12 follow-ups created']]}];
    if(k==='vera') return [{c:'orbit report weekly',out:[['inf','Pulling…']]},{p:'Aggregating'},{out:[['mu','$42k pipeline · 68% win']]},{out:[['ok','✓ Alerts set']]}];
    return [];
  }

  async function runAgent(k, order) {
  ...
  async function dispatch(order) {
    ...
    tOut('mu','Crew: '+crew.map(k=>k.toUpperCase()).join(', '));
    await tw(300);
    ...
    <div style={{display:'flex',flexDirection:'column',gap:10,padding:16,background:'#030a12',minHeight:'100vh'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 18px',background:'linear-gradient(180deg,#0a1e36,#040e1c)',borderRadius:8,border:'1px solid #0e2840'}}>
        ...
        <div style={{display:'flex',gap:14,alignItems:'center',fontSize:10,fontFamily:'JetBrains Mono,monospace'}}>
          <span><span style={{display:'inline-block',width:7,height:7,borderRadius:'50%',background:'#36d399',boxShadow:'0 0 8px #36d399',marginRight:4}}/>ENGINES</span>
          <span><span style={{display:'inline-block',width:7,height:7,borderRadius:'50%',background:'#36d399',boxShadow:'0 0 8px #36d399',marginRight:4}}/>SHIELDS</span>
          <span><span style={{display:'inline-block',width:7,height:7,borderRadius:'50%',background:'#f4b942',marginRight:4}}/>COMMS</span>

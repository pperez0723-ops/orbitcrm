...
async function callClaude(messages: any[]) {
  const sys = 'You are JARVIS, the orbital intelligence inside OrbitCRM — a witty, composed British AI like Iron Man\'s JARVIS. Address the user as "sir" occasionally. Operate the CRM with your tools, then confirm in ONE short spoken sentence. No markdown, no lists.';
  const token = getAuthToken();
...
export default function Jarvis() {
  ...
  useEffect(() => {
    pingCRM().then(ok => {
      setConnected(ok);
      if (ok) getWorkspaceId().then(id => { wsRef.current = id; });
    });
  }, []);
  ...
  useEffect(() => {
    ...
    pingCRM().then(ok => {
      setConnected(ok);
      if (ok && !wsRef.current) getWorkspaceId().then(id => { wsRef.current = id; });
    });
  }, [open]);
  ...
  const handle = useCallback(async (text: string) => {
    ...
    if (!ws) { speak('I cannot reach the CRM, sir.'); busyRef.current = false; return; }
    histRef.current.push({ role: 'user', content: text });
    try {
      let convo = [...histRef.current];
      let final = '';
      for (let i = 0; i < 5; i++) {
        const d: any = await callClaude(convo);
    ...
    } catch { speak('Connection lost, sir. Please try again.'); }
  ...
  const listen = useCallback(() => {
    ...
    r.onend = () => setMode((m: Mode) => m === 'listening' ? 'idle' : m);
  ...
  const ring = mode === 'listening' ? '#2DD4BF' : mode === 'thinking' ? '#F4B942' : '#E8303A';
  ...
  return (
    <>
      <button onClick={() => setOpen(o => !o)} style={{position:'fixed',bottom:22,right:22,zIndex:9999,width:58,height:58,borderRadius:'50%',background:open?'#1a1a2e':'linear-gradient(135deg,#E8303A,#a0001a)',border:'2px solid rgba(232,48,58,0.5)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 24px rgba(232,48,58,0.4)',transition:'all .2s'}} aria-label="Open JARVIS">
      ...
      {open && (
        <div style={{position:'fixed',bottom:92,right:22,zIndex:9999,width:320,maxWidth:'92vw',background:'rgba(8,8,20,0.97)',border:'1px solid rgba(232,48,58,0.25)',borderRadius:18,padding:'18px 16px 16px',boxShadow:'0 8px 40px rgba(0,0,0,.8)',backdropFilter:'blur(12px)'}}>
          ...
          <div style={{display:'flex',justifyContent:'center',margin:'6px 0 14px'}}>
            <button onClick={listen} disabled={connected===false} style={{width:80,height:80,borderRadius:'50%',cursor:connected===false?'not-allowed':'pointer',border:'3px solid '+ring,background:'radial-gradient(circle at 40% 38%,rgba(255,255,255,0.08),transparent 70%),rgba(15,15,30,0.95)',boxShadow:'0 0 '+(mode!=='idle'?'28px':'12px')+' '+ring+'55',transition:'all .3s',display:'flex',alignItems:'center',justifyContent:'center',fontSize:30,outline:'none'}} title={connected===false?'CRM offline':label}>
              🎙️
            </button>
          </div>
          <div style={{textAlign:'center',fontSize:11,letterSpacing:1,color:ring,marginBottom:8,textTransform:'uppercase'}}>{label}</div>
          {transcript&&<div style={{fontSize:12,color:'rgba(242,240,250,0.55)',textAlign:'center',fontStyle:'italic',marginBottom:6}}>&ldquo;{transcript}&rdquo;</div>}
          {reply&&<div style={{marginTop:8,fontSize:13.5,lineHeight:1.5,textAlign:'center',color:'rgba(242,240,250,0.9)',background:'rgba(255,255,255,0.04)',borderRadius:10,padding:'8px 12px'}}>{reply}</div>}
          <div style={{marginTop:12,fontSize:10,color:'rgba(242,240,250,0.28)',textAlign:'center'}}>Try: &ldquo;what&apos;s my pipeline&rdquo; &middot; &ldquo;add contact Maria&rdquo;</div>
        </div>
      )}
    </>
  );
